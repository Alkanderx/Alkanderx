(() => {
  ...
  const STORAGE_KEY = 'alkanderFullLayoutEditorV12';
  ...
  document.addEventListener('click', e => {
    ...
    if(e.target === toggle || e.target.closest('#layoutContextMenuV6,#layoutCopyOverlayV6')) return;
    e.preventDefault();
    e.stopImmediatePropagation();
  }, true);
  window.addEventListener('resize', positionHandles, true);
  ...
  function changeZ(kind){
    ...
    if(kind==='forward')z++;
    if(kind==='backward')z--;
    state.selected.style.zIndex=String(z);saveSelected();commit();updateFields();positionHandles();syncElementPopup();
  }
  ...
  function select(el){finishTextEditing();if(state.selected)state.selected.classList.remove('layout-selected-v6');removeHandles();state.selected=el;
    if(!el){if(ui.selected)ui.selected.textContent='Nothing selected';updateFields();syncElementPopup();return;}
    el.classList.add('layout-selected-v6');createHandles();if(ui.selected)ui.selected.textContent=describe(el);updateFields();syncElementPopup();
  }
  function createHandles(){
    if(!state.selected||isLocked(state.selected))return;
    for(const edge of ['nw','n','ne','e','se','s','sw','w']){
      const h=document.createElement('span');
      h.className=`layout-resize-handle-v6 edge-${edge}`;
      h.dataset.edge=edge;
      h.dataset.layoutHandleV12='true';
      document.body.appendChild(h);
    }
    positionHandles();
  }
  function removeHandles(){document.querySelectorAll('[data-layout-handle-v12]').forEach(h=>h.remove());}
  function positionHandles(){
    ...
    document.querySelectorAll('[data-layout-handle-v12]').forEach(h=>{
      const p=points[h.dataset.edge]; if(!p)return;
      h.style.left=`${Math.round(p[0]-6)}px`;h.style.top=`${Math.round(p[1]-6)}px`;
    });
  }
  ...
  function setElementText(value){
    if(!state.selected || !canEditText(state.selected) || isLocked(state.selected)) return;
  ...
  function syncElementPopup(){
    if(!ui.elementPopup || ui.elementPopup.closed || !ui.elementToolbar) return;
    const el = state.selected;
    const doc = ui.elementPopup.document;
    const status = ui.elementToolbar.querySelector('.element-selected-v11');
    const fields = ui.elementToolbar.querySelectorAll('[data-efield]');
    const textWrap = ui.elementToolbar.querySelector('.element-text-wrap-v11');
    const textArea = ui.elementToolbar.querySelector('[data-etext]');
    if(!el){
    ...
    if(status) status.textContent = describe(el);
    const rect = el.getBoundingClientRect();
    const vals = {
      x: parseFloat(el.style.left) || Math.round(rect.left),
      y: parseFloat(el.style.top) || Math.round(rect.top),
      w: Math.round(rect.width),
      h: Math.round(rect.height),
      z: parseInt(getComputedStyle(el).zIndex,10) || 0,
      font: parseFloat(getComputedStyle(el).fontSize) || 16,
      opacity: parseFloat(getComputedStyle(el).opacity) || 1
    };
    Object.entries(vals).forEach(([k,v]) => {
      const input = ui.elementToolbar.querySelector(`[data-efield="${k}"]`);
      if(input && doc.activeElement !== input) input.value = v;
    });
    if(textWrap){
      const editable = canEditText(el);
      textWrap.style.display = editable ? 'block' : 'none';
      if(editable && textArea && doc.activeElement !== textArea) textArea.value = el.textContent || '';
    }
  }
  ...
  function updateFields(){const el=state.selected,r=el?.getBoundingClientRect(),vals={x:el?(parseFloat(el.style.left)||Math.round(r.left)):'',y:el?(parseFloat(el.style.top)||Math.round(r.top)):'',w:el?Math.round(r.width):'',h:el?Math.round(r.height):'',z:el?(parseInt(getComputedStyle(el).zIndex,10)||0):'',font:el?(parseFloat(getComputedStyle(el).fontSize)||16):'',opacity:el?parseFloat(getComputedStyle(el).opacity):''};
    if(ui.toolbar)Object.entries(vals).forEach(([k,v])=>{const input=ui.toolbar.querySelector(`[data-field="${k}"]`);if(input)input.value=v;});
    syncElementPopup();
  }
  ...
  function applyRecords(){const sid=state.selected?.dataset.layoutIdV6;document.querySelectorAll('[data-layout-id-v6]').forEach(el=>restore(el,state.records[ensureId(el)]||state.baseline[ensureId(el)]));select(sid?document.querySelector(`[data-layout-id-v6="${sid}"]`):null);saveRecords();refreshOutput();syncElementPopup();}
  ...
  function exportData(){const changes=changed();return JSON.stringify({format:'Alkander full layout editor v12',viewport:{width:1368,height:801},generatedAt:new Date().toISOString(),changedElementCount:Object.keys(changes).length,changes},null,2);}
  ...
  function buildUI(){
    ...
    function openPopup(){
      ...
      api.toolbar.addEventListener('click',e=>{
        ...
        if(a==='download'){
          ...
          l.href=u;l.download='Alkander-layout-output-v12.json';l.click();URL.revokeObjectURL(u)
        }
        if(a==='reset')resetAll();
      });
    ...
    function openElementPopup(target){
      if(!state.active) return false;
      select(target || state.selected);
      if(api.elementPopup && !api.elementPopup.closed){
        syncElementPopup();
        api.elementPopup.focus();
        return true;
      }
      const pop = window.open('', 'AlkanderElementEditorPopup', 'popup=yes,width=360,height=650,resizable=yes,scrollbars=yes');
      if(!pop){ toast('Allow popups to open the element editor'); return false; }
      api.elementPopup = pop;
      const doc = pop.document;
      doc.open();
      doc.write(`<!doctype html><html><head><meta charset="utf-8"><title>Element Editor</title><style>
        *{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#080a10;color:#eee4d5;font-family:Arial,sans-serif}
        body{padding:12px;background:linear-gradient(155deg,#171322,#080a10)}
        #elementEditorV11{width:100%;min-height:100%;padding:0;background:transparent;color:#eee4d5}
        .element-head-v11{display:flex;justify-content:space-between;align-items:center;color:#e4c78f;font-family:Georgia,serif;font-size:17px;border-bottom:1px solid #70512e;padding-bottom:8px}
        .element-head-v11 button{border:0;background:none;color:#fff;font-size:24px;cursor:pointer}
        .element-selected-v11{padding:8px;margin:9px 0;border:1px solid #65502f;background:#090b12;color:#dbb2ee;font-size:10px;word-break:break-word;min-height:34px}
        p{font-size:11px;line-height:1.45;color:#c6bba9}
        .element-fields-v11{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.element-fields-v11 label{font-size:10px;color:#d4b376}
        input,textarea{display:block;width:100%;margin-top:3px;padding:6px 4px;background:#06080d;border:1px solid #5f4930;color:#fff}
        input{text-align:center}textarea{height:150px;resize:vertical;text-align:left}
        .element-buttons-v11{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:10px}.element-buttons-v11 button{padding:8px 4px;border:1px solid #72522e;background:#17141e;color:#eee2d0;border-radius:4px;font-size:11px;cursor:pointer}
        .element-buttons-v11 button:hover{background:#2c1d3a;border-color:#aa77cc}
      </style></head><body><aside id="elementEditorV11">
        <div class="element-head-v11"><strong>ELEMENT EDITOR</strong><button data-action="close">×</button></div>
        <p>Double-clicking an element opens this separate window. Changes here update the main character sheet live.</p>
        <div class="element-selected-v11">Nothing selected</div>
        <div class="element-fields-v11">
          <label>X<input data-efield="x" type="number"></label>
          <label>Y<input data-efield="y" type="number"></label>
          <label>W<input data-efield="w" type="number"></label>
          <label>H<input data-efield="h" type="number"></label>
          <label>Z<input data-efield="z" type="number"></label>
          <label>Font<input data-efield="font" type="number"></label>
          <label style="grid-column:1 / -1">Opacity<input data-efield="opacity" type="number" min="0" max="1" step="0.05"></label>
        </div>
        <div class="element-text-wrap-v11" style="margin-top:10px;display:none">
          <label style="font-size:10px;color:#d4b376">Text<textarea data-etext></textarea></label>
        </div>
        <div class="element-buttons-v11">
          <button data-action="front">Bring to front</button>
          <button data-action="back">Send to back</button>
          <button data-action="forward">Move forward</button>
          <button data-action="backward">Move backward</button>
          <button data-action="lock">Lock / unlock</button>
          <button data-action="reset">Reset item</button>
          <button data-action="hide">Hide element</button>
          <button data-action="focus">Focus main sheet</button>
        </div>
      </aside></body></html>`);
      doc.close();
      api.elementToolbar = doc.getElementById('elementEditorV11');
      api.elementToolbar.addEventListener('click', e => {
        const a = e.target.closest('[data-action]')?.dataset.action;
        if(!a) return;
        if(a === 'close'){ closeElementPopup(); return; }
        if(a === 'front') changeZ('front');
        if(a === 'back') changeZ('back');
        if(a === 'forward') changeZ('forward');
        if(a === 'backward') changeZ('backward');
        if(a === 'lock') toggleLock();
        if(a === 'reset') resetSelected();
        if(a === 'hide') hideSelected();
        if(a === 'focus') window.focus();
        syncElementPopup();
      });
      api.elementToolbar.querySelectorAll('[data-efield]').forEach(i => {
        i.addEventListener('input', () => {
          const value = Number(i.value);
          if(Number.isFinite(value)) applyField(i.dataset.efield, value);
        });
      });
      const textArea = api.elementToolbar.querySelector('[data-etext]');
      if(textArea){
        textArea.addEventListener('input', () => setElementText(textArea.value));
      }
      pop.addEventListener('beforeunload', () => {
        api.elementPopup = null;
        api.elementToolbar = null;
      });
      syncElementPopup();
      pop.focus();
      return true;
    ...
    function closeElementPopup(){
      if(api.elementPopup && !api.elementPopup.closed) api.elementPopup.close();
      api.elementPopup = null;
      api.elementToolbar = null;
    }
